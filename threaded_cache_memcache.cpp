...
#include <cstdlib>
#include <pthread.h>
//maybe there's a way to create a header file for generator to reuse some of the functions from there? That would probably be more efficient, but for now, we'll keep it separate
...
void *threaded_latencies(void *threadarg) {
    ...
    for (int i=0; i< (int) local_data->iters; i++) {
        ...
        local_data->time.push_back(duration / 1000);
        if (success)
            local_data->hits += 1;
        local_data->total += 1;
    }
...
lat_data threaded_performance(int nthreads, int iters, std::vector<std::string> pool, std::string host, std::string port) {
...
void thread_plot(int iters, int pool_size, double pool_ratio, int key_length, std::string host, std::string port) {
    ...
    for(int i=1; i<10; i++) {
        ...
        delete x;
        lat_data vals = threaded_performance(i, (int) iters / i, pool, host, port);
...
void thread_test(int threads, int iters, int pool_size, double pool_ratio, int key_length, std::string host, std::string port){
    ...
    warm_up(x, pool, pool_ratio);
    delete x;
    lat_data result = threaded_performance(threads, (int) iters / threads, pool, host, port);
